...
Remove the deprecated `--session` CLI alias. Re-review `/resume` against Pi and Grok Build, then tighten Zi's local owner composition, stale catalog bounds, no-op current selection, cancellation settlement state, torn-tail preview consistency, docs, and behavior tests without importing cross-project/remote/actor complexity.
