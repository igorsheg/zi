...
- `PromptStore` owns retained images, typed command/model workflows, and cursor-targeted one-shot composer edit requests. It receives typed edits or command intents from the mode-owned `SlashController`, projects one-line outcomes through narrow built-in notice actions, and delegates prompt, queue, catalog, and mutation operations through `InteractiveStore` to `AgentSession`.
