...
OpenZi uses `@opentui/core` directly. `@opentui/react`, React, JSX, and `@nanostores/react` are not runtime or development dependencies.
...
OpenZi follows that ownership model while keeping the terminal mode in its dedicated frontend package:
