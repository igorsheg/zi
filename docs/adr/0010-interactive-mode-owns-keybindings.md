...
Pi uses the same responsibility split at `0e6909f0`: `packages/coding-agent/src/core/keybindings.ts` defines semantic IDs, defaults, overrides, migration, and conflicts; `InteractiveMode` owns one effective manager; `CustomEditor` applies contextual precedence; and `ExtensionRunner.getShortcuts()` protects reserved bindings and reports conflicts. Zi keeps the owner in `packages/tui` because terminal behavior is frontend-specific, and deliberately rejects Pi TUI's mutable global keybinding singleton.
