...
- **Settings:** defaults are overlaid by global settings, project settings, then runtime/session/CLI overrides. `update()` writes global settings; `updateProject()` writes project settings. Locked read-modify-write preserves fields unknown to this version. If `<cwd>/.openzi` is the global directory (for example, cwd is `$HOME`), the identical file is loaded once and both write APIs update that one scope.
- **Authentication:** credentials are global only at `auth.json`. `FileCredentialStore` implements Pi AI's credential-store contract with bounded cross-process locking, atomic writes, and owner-only file permissions.
