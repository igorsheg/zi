...
Strict resume and continue-recent remain distinct intents. Resume must open an existing journal. Continue-recent may prepare a new persisted session when the current cwd has no saved session. The runtime retains the initial `ZiPaths.globalDir` as its canonical agent directory, so replacement never re-resolves omitted or relative configuration from ambient cwd, home, or environment state. `/new` uses the current runtime's resolved `ZiPaths.sessionDir`, not the directory containing a resumed journal, so an explicit session-directory override remains authoritative across replacement. Bootstrap precedence and the pending-to-durable journal transition are defined in ADR 0016. Unprompted and response-less sessions never enter the catalog. A malformed unterminated final JSONL append is treated as a torn tail by both full loading and catalog preview; malformed completed records still invalidate the journal.
