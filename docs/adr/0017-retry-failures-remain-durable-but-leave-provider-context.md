...
Retry waiting is an explicit phase of the owning agent run or compaction operation. That phase owns attempt identity, deadline, cancellation, and settlement. OpenZi accepts at most three retries and a 17-second base delay, keeping the complete `1x + 2x + 4x` wait below two minutes. Escape uses the mode-owned semantic interrupt path; the TUI derives its countdown from the session deadline through the existing renderer live lifecycle and owns no retry timer.
