...
Every native Zi runtime exposes `code` alongside its admitted direct tools. The name `code` is reserved before extension tool admission. Low-level SDK construction may omit the capability only by not supplying its concrete `CodeMode` owner; the CLI has no feature flag.
...
The child owns one QuickJS runtime and context. The runtime is limited to 64 MiB memory, 512 KiB stack, and 500 ms uninterrupted guest bursts. The guest receives only frozen `console` methods and a `zi` proxy over an immutable admitted tool-name catalog. Bridge functions are removed after bootstrap. There is no ambient `process`, `Bun`, `require`, `fetch`, module import, filesystem, shell, credential, network, or raw protocol authority. The worker receives a reduced process environment and performs no effects itself.
...
Nested responses have the stable guest shape `{ text, details }`. Full nested results remain transient protocol data. Durable `code_mode` details retain at most 64 projected calls with bounded arguments, command/path/operation evidence, outcome, duration, and a short result, error, or current preview, plus bounded console logs. The outer tool is the single transcript identity. Coding-agent presentation projects this trace for all clients; the TUI renders the same bounded facts. Successful nested `read`, `write`, and `edit` calls contribute to compaction file accounting.
