...
- internal agent↔tui mailbox transport is request/snapshot-shaped for conversation rendering and snapshot-shaped for queued state. raw `AgentEvent` callbacks still exist as host-private agent-thread plumbing, but they are not the tui mailbox contract.
...
agent loop
  ...
  (`AgentEvent`, compaction/retry outcomes, committed messages)
   ...
   │  - publishes conversation snapshots                              │  │
   ...
   └───────────────┬───────────────────────────────────────┬───────────┘  │
      ...
      │ derives rows from          │          │ abort / session control  │  │
      │ owned snapshot, then       │          │ mailboxes                │  │
      │ reconciles render        │          │                          │  │
      └──────────────┬───────────┘          └──────────────────────────┘  │
                     │                                                    │
                     v                                                    │
...
| internal conversation snapshot transport | authoritative `ConversationSnapshotEnvelope`, `QueuedMessageSnapshot` | keep host-private. it exists so tui can render and reconcile safely from authoritative snapshots | `docs/runtime.md:12-19`, `docs/extensions-retained-objects.md:133-161`, `src/coding_agent/runtime_host.zig`, `src/tui/conversation_projection.zig` |
...
| `RuntimeHost` event/snapshot publication | runtime host binds raw `AgentEvent` and `SessionEvent`, publishes `ConversationSnapshotEnvelope` to the tui mailbox, and publishes `QueuedMessageSnapshot` separately | re-derived | internal conversation snapshot transport plus internal lifecycle plumbing | `src/coding_agent/runtime_host.zig:35-52`, `src/coding_agent/runtime_host.zig:178-193`, `src/coding_agent/runtime_host.zig:374-390`, `src/tui/interactive.zig:2930-3009`, `docs/runtime.md:12-19` |
...
| tui conversation projection | projection owns `view_snapshot` and `queued_snapshot`, derives transcript/render state from the authoritative snapshot, and reconciles render from that owned state | re-derived | internal conversation snapshot transport feeding tui-owned projection/render | `src/tui/conversation_projection.zig:128-215`, `src/tui/conversation_projection.zig:256-337`, `src/tui/conversation_projection.zig:412-445`, `src/tui/interactive.zig:1142-1180` |
...
- `ConversationSnapshotEnvelope` / `QueuedMessageSnapshot` as extension api
...
- this does not freeze internal snapshot, mailbox, or wrapper structs as public api.
