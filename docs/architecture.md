...
Built-in tools also own their framework-neutral display projection. `projectToolDisplay()` validates partial arguments and result details into a closed bash/read/write/edit/generic value; it never imports OpenTUI or retains presentation state. This keeps truncation paths, continuation notices, write content, and executed edit diffs beside the tools that define those semantics while allowing each client to own layout and interaction.
