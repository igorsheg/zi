...
The single-file Bun executable gives Markdown renderables an instance-scoped no-op TreeSitter client. OpenTUI's dynamically resolved parser worker is not embedded by Bun 1.3 standalone compilation; retrying that missing worker on every code highlight causes unbounded worker churn and can crash the runtime. Standalone fenced code therefore remains selectable plain text. Source development keeps OpenTUI highlighting.
