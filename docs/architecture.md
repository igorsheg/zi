...
- Each enabled `AgentSession` owns one native `SubagentSupervisor`. It owns parent-session-unique model-authored names as the sole child identity, direct-child admission, `ChildZiProcess` RPC subprocesses, universal child instructions, work-cycle revisions, the bounded completion mailbox, native journal evidence, semantic notifications, and bounded disposal. `spawn_subagent` requires `{ name, prompt }`; every later operation routes by that name. Built-in tools adapt this owner, and extensions do not contribute subagent catalogs or policy. RPC remains a single-session transport and exposes no `agent.*` topology methods. See [ADR 0027](adr/0027-session-owned-native-subagents.md).
