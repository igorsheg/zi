...
- `ResourceLoader` is the concrete cwd-bound filesystem discovery owner. Each bounded `load()` returns a new immutable `SessionResources` value containing system prompts, contextual instructions, skills, prompt templates, subagent profiles, and non-fatal diagnostics; the loader retains no mutable current catalog.
...
- When child execution is available, `AgentSession` owns one `SubagentSupervisor`. Markdown resources and programmatic extension registration feed one canonical profile catalog; a non-empty catalog activates the standard model-facing orchestration tools. The supervisor owns runtime names, direct-child admission, `ChildZiProcess` RPC subprocesses, work cycles, bounded completion retention, native journal evidence, containment, and disposal. Extensions may build optional custom orchestration through bounded versioned session operations. Extension reload replaces programmatic registrations without terminating admitted children. See [ADR 0029](adr/0029-subagent-profiles-share-session-owned-orchestration.md).
  ...
  -> InteractiveMode
      ...
      -> SessionScreen
          ...
          -> PromptView + PromptStore
              -> AuthCeremonyView -> BrowserOpener
              -> QueuedInputsView
              -> Composer
