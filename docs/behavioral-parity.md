...
AgentSession
  owns session policy, preflight, public event queue/drain, persistence, resources, tools

session_events
  owns public session event/value payloads, JSON shape, and owned snapshots
...
| Public event queue overflow is explicit | `AgentSessionEvent` listeners | `AgentSession` queue, `session_events` payloads | implemented | `agent session public event queue overflow is explicit`; `prompt command event serializes public shape` |
...
| TUI observes public session behavior only | interactive mode over `AgentSession` | `frontends/tui` adapter / `tui.ProductApp` | implemented first slice: programmatic mailbox adapter, no `src/tui -> coding_agent`, no `src/coding_agent -> tui/frontends`; recovery falls back to snapshot | build/test compile boundary; adapter mapping tests; gap/snapshot recovery test pending |
| Agent terminal events are facts, not snapshots | `AgentSessionEvent` lifecycle events plus session context APIs | `agent.AgentEvent` / `client_protocol` | implemented: `agent_end` and `turn_end` are marker-only; snapshots/replay carry state | `agent end is lifecycle marker without transcript payload`; lifecycle payload grep gate |
| Tool result display and model context are separate bounded projections | pi tool output accumulator + session context transformation | `agent.defaultConvertToLlm`, tools output policy | implemented first slice: model-context tool result text capped, images/details omitted, request arena freed per model call | `default llm conversion bounds tool result text`; end-to-end large tool output test pending |
| Auto compaction events and policy | `core/compaction/*` | `AgentSession` terminal policy, `session_events` payloads | protocol, durable bounded session entry shape, context projection, owned result event payload, deterministic preparation, owned summary input, bounded summary-input serialization, opt-in pre-prompt threshold compaction, terminal context-overflow classification, post-prompt overflow compaction, and one bounded overflow retry | `failed compaction end event omits result`; `compaction end event serializes owned result`; `append compaction stores durable summary entry`; `append compaction enforces bounded owned fields before mutation`; `append compaction rejects first kept entry outside active branch`; `loaded compaction entries reject first kept entry outside parent branch`; `session store round trips compaction entries`; `build context projects latest compaction summary then kept messages`; `prepare compaction keeps bounded recent suffix`; `build compaction summary input owns messages to summarize`; `build compaction summary input carries previous summary`; `serialize compaction summary input writes deterministic bounded text`; `serialize compaction summary input truncates tool results and bounds output`; `agent session auto compacts before prompt when enabled`; `agent session auto compaction skips already compacted branch before prompt`; `agent session auto compacts after context overflow and retries once`; `agent session overflow retry does not recurse on second overflow`; `agent session terminal policy classifies context overflow errors`; `agent session terminal policy ignores non context errors for overflow` |
| Manual compaction | `AgentSession.compact()` | `AgentSession` future terminal policy | prepared explicit-summary API with session-owned keep-recent setting, provider-backed generated summary, owned cancellation source, owned preparation and summary-input snapshots, and runtime host command | `agent session manual compaction persists and replaces agent context`; `agent session prepared manual compaction owns cutpoint selection`; `agent session prepared manual compaction uses session settings`; `agent session generated manual compaction summarizes and persists`; `agent session generated manual compaction failure does not mutate history`; `agent session generated manual compaction oversized summary does not mutate history`; `agent session generated manual compaction cancellation is observable`; `agent session prepares owned compaction snapshot`; `agent session prepares owned compaction summary input snapshot`; `agent session prepared manual compaction emits failure event`; `runtime host compacts through public command boundary`; `runtime host compacts with generated summary through public command boundary`; `runtime host exposes compaction preparation snapshot`; `runtime host exposes compaction summary input snapshot`; `session config uses project compaction settings before global settings`; `prepare compaction rejects empty small and already compacted branches`; `prepare compaction reuses previous first kept boundary` |
| Auto retry events and policy | `auto_retry_start/end` in `AgentSession` | `AgentSession` future terminal policy | one immediate overflow retry after successful overflow compaction; bounded transient-error continue retry with settings; backoff/cancel delay missing | `agent session auto compacts after context overflow and retries once`; `agent session overflow retry does not recurse on second overflow`; `agent session retries transient assistant errors through continue`; `agent session transient retry stops at bounded attempts` |
| Bash/process tool with timeout/cancel | `core/bash-executor.ts`, `tools/bash.ts` | `tools/BashTool` / `AgentSession` | minimal implemented over zio-native process runner: cwd-bound, sequential, timeout/cancel/output-limit result data; no stdin/env/PTY/background/streaming | `bash tool runs one cwd-bound command`; `bash tool treats nonzero exit as result data`; `bash tool treats timeout as bounded result data`; `bash tool treats output limit as bounded result data`; `bash tool cancels running process through owner race`; `runtime host preserves bash output limit details through public events`; `runtime host cancellation reaches running bash tool through agent loop` |
