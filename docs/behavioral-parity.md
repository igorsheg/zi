...
| Auto compaction events and policy | `core/compaction/*` | `AgentSession` future terminal policy | protocol, durable bounded session entry shape, context projection, owned result event payload, deterministic preparation, owned summary input, bounded summary-input serialization, opt-in pre-prompt threshold compaction, and terminal context-overflow classification; overflow recovery missing | `agent session failed compaction end event omits result`; `agent session compaction end event serializes owned result`; `append compaction stores durable summary entry`; `append compaction enforces bounded owned fields before mutation`; `append compaction rejects first kept entry outside active branch`; `loaded compaction entries reject first kept entry outside parent branch`; `session store round trips compaction entries`; `build context projects latest compaction summary then kept messages`; `prepare compaction keeps bounded recent suffix`; `build compaction summary input owns messages to summarize`; `build compaction summary input carries previous summary`; `serialize compaction summary input writes deterministic bounded text`; `serialize compaction summary input truncates tool results and bounds output`; `agent session auto compacts before prompt when enabled`; `agent session auto compaction skips already compacted branch before prompt`; `agent session terminal policy classifies context overflow errors`; `agent session terminal policy ignores non context errors for overflow` |
