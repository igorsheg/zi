...
| One owner for the live session and its mailbox | `core/agent-session-runtime.ts` | `SessionRuntime` | implemented; `SessionRuntime` is the stable mailbox host and switches sessions by building the next services/session slot before swapping | `session runtime owns services session and bounded mailboxes`; `session runtime switches to a selected session through the mailbox` |
...
| Model selection resolves explicit before session restore before settings | `model-resolver.ts`, `session-manager.ts` | `session_runtime.resolveSessionOptions` + `session_manager.sessionPreferences` | implemented: `/model` persists `model_change`; resume restores persisted model/thinking when no explicit override and auth/provider are available | `session runtime model command selects an authenticated model`; `session runtime switch restores persisted session model` |
...
| Bash/process tool with timeout/cancel | `core/bash-executor.ts`, `tools/bash.ts` | `tools/bash` | implemented over the zio process runner: cwd-bound bash via `/bin/bash -c` when available, then `bash -c`, with Unix `sh -c` fallback only when bash is unavailable; env passthrough, timeout/cancel/output-limit as result data, streamed bounded output previews; no stdin/PTY/background jobs | `bash tool runs one cwd-bound command`; `bash tool executes bash syntax on unix`; `bash tool treats timeout as bounded result data`; `bash tool treats output limit as bounded result data`; `bash tool cancels running process through owner race`; `process runner drains queued output to observer after fast process exit` |
| Slash commands | `slash-commands.ts` | `SessionRuntime` | implemented first slice (restored 2026-06-13 after being dropped in the mailbox refactor): `/help` and `/session` reply with request-correlated `prompt_command` events; `/model` updates model; `/resume` switches sessions through the mailbox; unknown commands reply `unknown`; never reaches the model or queues, active operation or not | `session runtime handles slash command without starting an operation`; `session runtime session command reports session facts`; `session runtime switches to a selected session through the mailbox`; `session runtime replies unknown for unrecognized slash command`; `session runtime slash command never queues while an operation is active`; `prompt command event serializes public shape` |
