...
| Auto compaction events and policy | `core/compaction/*` | `AgentSession` future terminal policy | protocol, durable session entry shape, and context projection only; pi-mono result payload (`summary`, `firstKeptEntryId`, `tokensBefore`, `details`) deferred until compaction policy owns real data; no dummy result payload | `agent session compaction end event serializes owned state only`; `append compaction stores durable summary entry`; `session store round trips compaction entries`; `build context projects latest compaction summary then kept messages` |
