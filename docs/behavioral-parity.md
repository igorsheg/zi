...
| Manual compaction | `AgentSession.compact()` | `AgentSession` future terminal policy | prepared explicit-summary API only; LLM summarization, cancellation controller, and settings policy missing | `agent session manual compaction persists and replaces agent context`; `agent session prepared manual compaction owns cutpoint selection`; `agent session prepared manual compaction emits failure event`; `prepare compaction rejects empty small and already compacted branches`; `prepare compaction reuses previous first kept boundary` |
