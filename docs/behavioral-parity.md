...
| Manual compaction | `AgentSession.compact()` | `AgentSession` future terminal policy | prepared explicit-summary API with session-owned keep-recent setting, owned preparation and summary-input snapshots, and runtime host command; LLM summarization and cancellation controller missing | `agent session manual compaction persists and replaces agent context`; `agent session prepared manual compaction owns cutpoint selection`; `agent session prepared manual compaction uses session settings`; `agent session prepares owned compaction snapshot`; `agent session prepares owned compaction summary input snapshot`; `agent session prepared manual compaction emits failure event`; `runtime host compacts through public command boundary`; `runtime host exposes compaction preparation snapshot`; `runtime host exposes compaction summary input snapshot`; `session config uses project compaction settings before global settings`; `prepare compaction rejects empty small and already compacted branches`; `prepare compaction reuses previous first kept boundary` |
