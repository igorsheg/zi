...
| Manual compaction | `AgentSession.compact()` | `AgentSession` future terminal policy | prepared explicit-summary API with session-owned keep-recent setting, owned preparation snapshot, and runtime host command; LLM summarization and cancellation controller missing | `agent session manual compaction persists and replaces agent context`; `agent session prepared manual compaction owns cutpoint selection`; `agent session prepared manual compaction uses session settings`; `agent session prepares owned compaction snapshot`; `agent session prepared manual compaction emits failure event`; `runtime host compacts through public command boundary`; `runtime host exposes compaction preparation snapshot`; `session config uses project compaction settings before global settings`; `prepare compaction rejects empty small and already compacted branches`; `prepare compaction reuses previous first kept boundary` |
