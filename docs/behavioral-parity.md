...
| Auto compaction events and policy | `core/compaction/*` | `AgentSession` future terminal policy | protocol only; pi-mono result payload (`summary`, `firstKeptEntryId`, `tokensBefore`, `details`) deferred until compaction owns real data; no dummy result payload | `agent session compaction end event serializes owned state only` |
