...
| Auto compaction events and policy | `core/compaction/*` | `AgentSession` future terminal policy | protocol, durable session entry shape, context projection, owned result event payload, and deterministic preparation; details, summarization, and auto policy missing | `agent session failed compaction end event omits result`; `agent session compaction end event serializes owned result`; `append compaction stores durable summary entry`; `session store round trips compaction entries`; `build context projects latest compaction summary then kept messages`; `prepare compaction keeps bounded recent suffix` |
