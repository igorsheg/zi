...
| Manual compaction | `AgentSession.compact()` | `AgentSession` future terminal policy | explicit-summary API and deterministic preparation only; LLM summarization, cancellation controller, and settings policy missing | `agent session manual compaction persists and replaces agent context`; `prepare compaction rejects empty small and already compacted branches`; `prepare compaction reuses previous first kept boundary` |
