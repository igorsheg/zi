...
| Auto retry events and policy | `auto_retry_start/end` in `AgentSession` | `AgentSession` future terminal policy | one immediate overflow retry after successful overflow compaction; general retryable errors and backoff missing | `agent session auto compacts after context overflow and retries once`; `agent session overflow retry does not recurse on second overflow` |
