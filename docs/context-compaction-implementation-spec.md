...
The primary decision is deliberate: OpenZi should port Pi's append-only, tail-preserving compaction shape, not Grok Build's whole-session replacement architecture. Grok Build contributes failure, accounting, validation, and preflight lessons. It does not replace Pi as the coding-agent behavior and architecture reference.
...
- compaction is admitted by OpenZi's explicit `AgentSession` activity states; manual compaction does not silently abort unrelated work;
...
- persisted live-state reconstruction for resources OpenZi already supplies through the current system prompt;
...
| Concern           | Pi                                            | Grok Build                                     | OpenZi decision                                                  |
| ----------------- | --------------------------------------------- | ---------------------------------------------- | ---------------------------------------------------------------- |
...
| UI                | Compaction status + rebuilt transcript        | Notifications and context diagnostics          | OpenZi-owned status, usage, and active transcript reset          |
...
8. A split inside one user turn is allowed. The discarded prefix is summarized with an explicit instruction that the exact recent suffix remains; OpenZi uses one iterative summary stream rather than Pi's second turn-prefix request.
...
Use flat OpenZi-owned settings so global/project layering stays shallow and obvious:
...
Suppression is run-local in the first milestone. Grok Build's persistent reason-scoped suppression is valuable at its scale, but OpenZi should not persist a failure policy before repeated real-world failures demonstrate the need.
