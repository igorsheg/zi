...
| `model-status.ts` | model-select status item plus notification | shipped | `examples/extensions/model_status.lua` observes `model_select`, publishes a host-owned notification for non-restore changes, and updates keyed `ctx.ui.status({ id = "model", ... })` | closed in `zi-3n5c` on top of notification surface `zi-noyv`. |
| `widget-placement.ts` | widgets above/below editor | shipped | `examples/extensions/widget_placement.lua` handles `session_start` and publishes above/below editor widgets through host-owned semantic status/report updates | closed in `zi-0scq`; richer arbitration/styling remains future scope. |
| `hidden-thinking-label.ts` | customize collapsed thinking label | shipped | `examples/extensions/hidden_thinking_label.lua` publishes a compact `ctx.ui.status` item on session start and through `/thinking-label`; TUI assistant-message rendering uses that label for hidden thinking blocks | closed in `zi-vjan`. |
| `custom-header.ts` | claim/restore header | shipped | `examples/extensions/custom_header.lua` claims the header on `session_start` with host-owned `ctx.ui.report` payloads and registers `/builtin-header` to restore the built-in header surface | closed in `zi-uo7u`; raw component factories remain deliberately excluded. |
| `custom-footer.ts` | claim footer with dynamic branch/model/token data | partial | semantic message/status/report intents; full parity also needs semantic footer data helpers or renderer refs for git branch/status/token/model data | richer renderer refs, footer data helpers, and conflict arbitration. |
