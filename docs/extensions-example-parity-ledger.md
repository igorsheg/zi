...
| `built-in-tool-renderer.ts`, `minimal-mode.ts` | override builtin tool rendering | partial | `render_result` is the first tool `result` presentation-slot vertical: Lua returns a host-owned line/span presentation document, custom renderers win over fallback, and empty collapsed output supports minimal-mode semantics; `examples/extensions/minimal_mode.lua` demonstrates a builtin-name `read` override with custom result presentation | remaining `zi-0j8`: general renderer refs / call-slot presentation / broader message-renderer registry. |
...
3. **tool surface:** `tool-override.ts` equivalent is shipped and the first tool result presentation-slot vertical supports minimal-mode/custom result rendering; remaining work is call-slot presentation, explicit renderer refs, and broader builtin/message renderer registry semantics.
