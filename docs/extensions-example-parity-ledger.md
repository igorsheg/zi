...
3. **tool surface:** `tool-override.ts` equivalent is shipped and the first tool call/result presentation-slot verticals support custom builtin-name rendering plus minimal-mode result semantics; remaining work is explicit renderer refs and broader builtin/message renderer registry semantics.
