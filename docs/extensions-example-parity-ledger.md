...
| `hidden-thinking-label.ts` | customize collapsed thinking label | shipped | `examples/extensions/hidden_thinking_label.lua` publishes the singleton `ctx.ui.set_hidden_thinking_label` surface on session start and through `/thinking-label`; TUI assistant-message rendering uses that label for hidden thinking blocks | closed in `zi-vjan`. |
...
| `titlebar-spinner.ts` | terminal title changes while the agent works | partial | `examples/extensions/titlebar.lua` publishes host-owned `ctx.ui.set_title` intents on lifecycle events, and the TUI materializes them through its terminal title backend | static working/idle title parity is closed in `zi-ugxf`; animated timer-driven spinner still needs a scheduler/timer seam. |
...
| `question.ts` | interactive tool asks user to select one answer | shipped | `examples/extensions/question.lua` registers a `question` tool that uses host-owned `ctx.ui.select` prompt requests and semantic tool details/rendering instead of raw custom UI | closed in `zi-g8y2`; raw custom UI remains deliberately excluded. |
