...
`ctx.ui` also exposes host-owned semantic UI methods from bound extension contexts, including `message`, `status`, `progress`, `report`, `prompt`, `pick`, and editor actions.
...
| `zi-rep0.4` | closed: first host-owned command output/report primitive (`ctx.ui.report`) for `/todos`-class commands. |
| `zi-rep0.5` | closed: prompt family request boundary (`ctx.ui.prompt`/`ctx.ui.pick`) with host-owned records and default unbind cancellation. |
| `zi-rep0.6` | closed: retained semantic message/status/progress/report surface intents with TUI snapshot consumption. |
...
| `zi-rep0.8` | retired: raw overlay surface leases are not public API; use semantic prompt/report/pick primitives. |
...
| `todo.ts` | custom tool + command + persisted state + custom rendering + command output UI | shipped | `examples/extensions/todo.lua` ships tool, command registration, branch-replayed `ctx.state` persistence, tool-result details, `render_result`, and `/todos` host-owned command report output via `ctx.ui.report` | closed in `zi-rep0.4`. |
...
| `commands.ts` | slash command registration | shipped | `examples/extensions/commands.lua` registers `/hello`, dispatches on the agent-owned command path, and publishes host-owned report output through `ctx.ui.report` | closed in `zi-ovmg`. |
...
| `plan-mode/` | command-driven mode with tool restrictions and progress ui | blocked | commands + retained mode state + status/progress/report + tool policy | after commands, flags, semantic ui records, and tool-policy hooks. |
...
| `handoff.ts` | command + custom loader ui + editor prompt + session handoff | partial | command + progress/message publication + editor modal request + editor buffer action + handoff host action | handoff action. |
| `qna.ts` | prompt-generator command: extract questions from last assistant message, run model completion, show working/notification UI, then mutate editor buffer | partial | `examples/extensions/qna.lua` is only a seed that proves command-driven editor mutation through host-owned `ctx.ui.set_editor_text`; full qna parity needs semantic branch/message reads, extension-initiated model completion without exposing raw provider auth, message/progress UI, and async/cancellable command execution | `zi-2wn2` closed only the editor-action sub-capability; create a full qna bead after those seams exist. |
| `status-line.ts` | keyed status item from lifecycle/turn events | shipped | `examples/extensions/status_line.lua` publishes a `status-demo` item on `session_start`, `turn_start`, and `turn_end` through host-owned `ctx.ui.status` updates | closed in `zi-1bmm`; richer styling remains future scope. |
| `model-status.ts` | model-select status item plus notification | shipped | `examples/extensions/model_status.lua` observes `model_select`, publishes host-owned short feedback for non-restore changes, and updates keyed `ctx.ui.status({ id = "model", ... })` | closed in `zi-3n5c` on top of semantic message feedback. |
| `widget-placement.ts` | widgets above/below editor | shipped | `examples/extensions/widget_placement.lua` remains a pi-mono parity example, but publishes host-owned semantic status/report updates rather than widget slots | closed in `zi-0scq`; richer arbitration/styling remains future scope. |
| `hidden-thinking-label.ts` | customize collapsed thinking label | shipped | `examples/extensions/hidden_thinking_label.lua` is now a pi-mono parity example that publishes compact `ctx.ui.status` feedback; raw hidden-thinking-label mutation is not public API | closed in `zi-vjan`. |
| `custom-header.ts` | claim/restore header | shipped | `examples/extensions/custom_header.lua` remains a pi-mono parity example, but publishes host-owned `ctx.ui.report` payloads rather than claiming a header slot | closed in `zi-uo7u`; raw component factories remain deliberately excluded. |
| `custom-footer.ts` | claim footer with dynamic branch/model/token data | partial | semantic message/status/report intents; full parity also needs semantic context data helpers or renderer refs for git branch/status/token/model data | richer renderer refs, context data helpers, and conflict arbitration. |
| `summarize.ts` | command/event producing transient ui output | blocked | command + retained report/message/status/progress + model/tool access | semantic ui publications + context usage/model helpers. |
...
| `notify.ts` | notify when agent is ready for input | shipped | `examples/extensions/message.lua` publishes `ctx.ui.message` as host-owned short feedback from `agent_end`; TUI materializes as host-owned short feedback; terminal-native notification backends remain host policy | closed in `zi-noyv`; OS-specific non-terminal toast backends remain optional future polish. |
| `titlebar-spinner.ts` | terminal title changes while the agent works | partial | `examples/extensions/titlebar.lua` publishes host-owned `ctx.ui.status` intents on lifecycle events, and the TUI materializes them through its terminal title backend | static working/idle title parity is closed in `zi-ugxf`; animated timer-driven spinner still needs a scheduler/timer seam. |
...
| `question.ts` | interactive tool asks user to select one answer | shipped | `examples/extensions/question.lua` registers a `question` tool that uses host-owned `ctx.ui.pick` prompt requests and semantic tool details/rendering instead of raw custom UI | closed in `zi-g8y2`; raw custom UI remains deliberately excluded. |
| `questionnaire.ts` | multi-step interactive questionnaire | partial | prompt/select/input/editor request records callable from lua tool/event contexts | needs runnable multi-step example and rpc-host materialization. |
| `overlay-test.ts`, `overlay-qa-tests.ts`, `doom-overlay/`, `snake.ts`, `space-invaders.ts` | advanced custom overlays / games / keyboard-driven ui | partial | semantic prompt/report/pick primitives; raw overlays are no longer public; future renderer refs/focus leases/input intents stay host-owned | richer overlay materialization, renderer refs, and constrained host-owned input intents. |
