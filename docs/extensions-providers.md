...
current implementation slice: `zi.register_provider(name, { api?, base_url, api_key?, headers?, models?, oauth? })` and `zi.unregister_provider(name)` now queue before bind, apply immediately after bind, revoke on unbind/teardown, and drive a host-owned visible model catalog rebuild from active provider claims. claim-backed models still resolve provider-level `api_key`, `headers`, and minimal built-in-template-backed oauth/login through the host runtime without exposing that config on visible model snapshots. built-in visible provider names with runtime truth today (`anthropic`, `openai`, `openrouter`, `openai-codex`) now support override-only registration with inferred `api`; that path rejects `models` and `oauth`, preserves the built-in visible catalog, and scopes runtime base-url overrides by provider name instead of leaking across providers that share an api family. `auth_header`, built-in model replacement, extension-defined oauth callbacks, and custom stream handlers are still follow-up work.
