...
current implementation slice: `zi.register_provider(name, { api, base_url, api_key?, headers?, models? })` and `zi.unregister_provider(name)` now queue before bind, apply immediately after bind, revoke on unbind/teardown, and drive a host-owned visible model catalog rebuild from active provider claims. claim-backed models now resolve provider-level `api_key` and `headers` through the host runtime without exposing that config on visible model snapshots. `auth_header`, oauth/login work, and custom stream handlers are still follow-up work.
