...
current implementation slice: `zi.register_provider(name, { api?, base_url, api_key?, headers?, models?, oauth? })` and `zi.unregister_provider(name)` now queue before bind, apply immediately after bind, revoke on unbind/teardown, and drive a host-owned visible model catalog rebuild from active provider claims. claim-backed models resolve provider-level `api_key`, `headers`, and host-backed oauth through the runtime without exposing provider config on visible model snapshots. extension oauth supports built-in-template login plus extension-defined `login`, `refresh_token`/`refreshToken`, and `getApiKey` callbacks; non-agent threads request callback work through the agent-owned request seam instead of calling lua directly. built-in visible provider names with runtime truth today (`anthropic`, `openai`, `openrouter`, `openai-codex`) support override-only registration with inferred `api`; that path rejects `models` and `oauth`, preserves the built-in visible catalog, and scopes runtime base-url overrides by provider name instead of leaking across providers that share an api family. `auth_header`, built-in model replacement, `oauth.modifyModels`, and custom stream handlers are still follow-up work.
