...
| B6 ESC abort | `Loop.zig` test "run cancel restores queued text into empty editor"; pty e2e "streamed markdown renders, escape aborts, and ctrl-c exits" |
| B7 tool expand/collapse | `Loop.zig` test "loop collapsed tool body ending newline has no blank before marker" |
| B8 typing during streaming and steer/follow-up | `Loop.zig` test "run driver queues steering and dequeue-all restores queued text" |
| B9 queued steering/follow-up display and Alt+Q | `Loop.zig` test "run driver queues steering and dequeue-all restores queued text" |
| B10 ESC cascade | `Loop.zig` cancel/retry/compaction paths; pty e2e abort coverage |
| B11 Ctrl+C/Ctrl+D exit | `Loop.zig` test "loop clear_or_quit clears first then exits"; `root.zig` test "frame loop step exits through pumped ctrl-d input" |
...
| B15 model/session pickers | `Loop.zig` test "loop P4 model picker applies faux model through services"; pty e2e "P4 completion model picker resume and new session" |
| B16 session restore fold | `Loop.zig` test "loop P4 restore fold renders durable user tool and assistant transcript"; `Loop.zig` test "loop P4 header shows compaction count" |
...
| B19 working status | `Loop.zig` RunDriver faux session coverage and pty streaming e2e |
...
| B24 resize | `root.zig` test "runner tick renders immediately on resize"; pty e2e "P3 viewport rebuild and resize storm" |
| B25 exit hygiene | `Terminal.zig` test "terminal setup and shutdown are idempotent"; pty e2e termios assertions; panic restore via `main.zig` panic handler |
...
| B27 editor behavior | `Editor.zig` grapheme/delete/history/kill/undo tests; `InputDecoder.zig` bracketed paste and keymap tests |
