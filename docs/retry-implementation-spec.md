...
This document maps Pi's retry behavior to Zi's current owners. It distinguishes visible coding-agent retries from provider/SDK retries and context-overflow recovery so the implementation does not collapse three different policies into one loop.

## Sources

Primary parity source:

- `earendil-works/pi` at Zi's pinned `0e6909f0` reference, including:
...
The post-pin commits are research inputs, not a silent change to `docs/reference-pins.md` or Zi's installed `@earendil-works/pi-*` `0.80.6` dependencies.
...
Zi's installed `pi-ai` already exports the classifier used by the pinned implementation. Pi current main adds one further early-EOF phrase for OpenAI Responses; adopting that requires a dependency refresh or a deliberate local compatibility decision.
...
Pi RPC additionally exposes `set_auto_retry` and `abort_retry`. Zi has deliberately deferred RPC, so those commands are not part of the first Zi slice. The client-independent session state and operations must still leave room for them.

## Pi behavior that Zi should not copy blindly

### Unbounded configured backoff

Pi reads numeric retry settings without the strict bounds Zi requires. Exponential delay can therefore become impractically large. Zi must validate persisted/runtime settings and bound attempts, each delay, cumulative waits, and the containing operation.
...
Zi resolves this with append-only retry markers and separate context/presentation projections. Failed attempts remain durable and visible but never re-enter provider context after retry, resume, or later compaction.

### Timer ownership

Pi's countdown component owns a one-second interval. Zi's terminal constraints prohibit an independent presentation timer. The countdown should derive seconds from the retry deadline during the existing renderer-owned live lifecycle and release that live request with the status owner.
...
- Zi accepts at most three retries and a 17-second base delay. The `1x + 2x + 4x` sequence is therefore always below two minutes; defaults remain Pi's 2, 4, and 8 seconds.
