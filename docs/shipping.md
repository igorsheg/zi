...
OpenZi has two distribution boundaries:
...
On Windows the output is `dist/openzi.exe`. The local build compiles to a temporary file, verifies its embedded version, and only then replaces the previous executable. Its version identifies the current commit and a dirty working tree, including untracked files, while `OPENZI_BUILD_VERSION` can provide an explicit local version. It does not create a release archive.
...
```sh
git tag -s v0.1.0 -m "OpenZi v0.1.0"
git push origin v0.1.0
```

Before the first public release, choose the OpenZi license, add the corresponding root license file and third-party notices, and configure macOS notarization and Windows signing. GitHub attestations prove workflow provenance but do not replace platform signatures.
