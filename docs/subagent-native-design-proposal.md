...
- declarative extension-provided agent definitions;
- explicit foreground/background ownership transfer;
- bounded completion records;
- dynamic model guidance from the admitted definition catalog.
...
- declarative named child definitions;
...
- agent nicknames and paths in place of simple opaque IDs;
...
- `spawn_subagent`;
- `send_subagent` — queue-only;
- `continue_subagent` — direct when idle, follow-up when running;
- `wait_subagents`;
- `interrupt_subagent`;
- `close_subagent`;
- `list_subagents`.
...
- queue-only send does not wake an idle child.
...
The native extension API is declarative:
...
The extension contributes policy data; Zi owns execution. Extension authors do not implement process launch, RPC, waiters, durability, or shutdown.

V1 definition fields are deliberately narrow:
...
V1 does not claim enforcement of read-only capability, permission derivation, worktree isolation, model routing, tool filtering, parent-context fork, or resumability. Those fields must not appear until their owners and behavior exist.

Registration follows existing extension rules:

- only during factory execution;
- bounded strings and count;
- deterministic source precedence;
- built-in names cannot be replaced;
- candidate reload is atomic;
- a running child retains the immutable definition snapshot admitted at spawn;
- removing a definition prevents new spawns but does not mutate a live child.

This proposal does **not** initially expose procedural `zi.subagents.spawn()` methods to extension code. Grok and Codex both demonstrate that the useful extension seam is definition/role registration, while the model-facing native tools own orchestration. A procedural API can be added later for a concrete deterministic workflow consumer; adding it now would commit lifecycle and cross-extension ownership semantics without evidence.

### Built-in definition

Zi initially ships one built-in type:

- `general` — normal coding-agent tools and authority.

Zi must not ship an `explore` or `reviewer` type described as read-only until child tool filtering actually enforces that restriction. Prompt-only restrictions are guidance, not authority.
...
│   ├── immutable subagent-definition snapshot per spawn
│   ├── child IDs and lifecycle transitions
...
`AgentSession` owns the capability because its lifetime, tool catalog, interruption policy, journal, and extension-definition catalog are all session policy. `SubagentSupervisor` remains a deep concrete module; it does not become a generic agent transport or session registry.
...
The parent also passes its admitted model and thinking level explicitly so a child does not silently select a different default. Credentials continue through normal Zi credential and provider environment policy.
...
Native subagents do not belong to an extension generation:

- `/reload` atomically replaces the available extension-provided definition catalog;
- existing children continue with their spawn-time snapshot;
- extension worker crash removes its definitions until reload but does not kill native children;
- parent session disposal closes all children.

This is the main ownership change from ADR 0026.
...
5. a later parent turn receives a short generated notice listing undelivered completed IDs and instructing the model to call `wait_subagents`.
...
`wait_subagents` returns the bounded full completion and marks that mailbox item delivered. Timeout returns current snapshots and never cancels children.
...
- maximum sixteen distinct IDs per wait;
- maximum twenty-five-second tool wait;
...
- `close_subagent` ends the child process and releases capacity;
...
`spawn_subagent.type` carries the current bounded type catalog as parameter documentation. Each line contains the exact registered name and a normalized, UTF-8-safe description; child instructions and extension source metadata never enter the model-facing catalog. Stable delegation policy remains in the system prompt rather than being repeated in extension-provided text.

`AgentSession` derives fresh native tools from the supervisor's authoritative definition snapshot whenever the extension host atomically publishes a new tool-and-type catalog. It then rebuilds the active model tools and Code Mode's immutable catalog. Existing children retain their spawn-time definition snapshots, while the next provider turn sees added or removed types.
...
- `subagents`;
- `subagentStatus`, including bounded working and ready agent identities;
...
Native tools project concise semantic rows such as `Started Reviewer`, `Finished waiting`, and `Closed Reviewer` through the coding-agent `ToolPresentation` boundary. Typed result details preserve model-facing JSON while keeping protocol names, envelopes, and agent IDs out of compact transcript presentation.
...
7. [x] Add `registerSubagentType()` to the versioned extension API and worker protocol.
8. [x] Add client-neutral subagent snapshots and semantic events.
9. [x] Complete local race, cancellation, reload/crash-survival, mailbox, journal, and Code Mode acceptance against mock RPC children.
...
11. [x] Remove the installed/copyable orchestration implementation and retain only a small definition-registration example.

## Decision summary

Zi adopts the shared conclusion from Grok Build and Codex: subagent orchestration is native coding-agent policy, while extensions contribute declarative child roles/definitions.
