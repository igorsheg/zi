...
The standard tools and optional extension API support profile listing, spawn, queue-only send, continue, wait, list, interrupt, and close over the same supervisor. Spawn transfers admitted work to session ownership. Cancelling before prompt admission closes the starting child. Cancelling wait removes only the waiter. Interrupt stops active work while preserving the reusable process. Close terminates one child and releases its live slot.
