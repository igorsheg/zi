...
`send_subagent` queues context or information and never starts an idle turn. Use `continue_subagent` to assign work: it starts an idle turn or adds the task to the active work cycle. Their successful model-facing results are concise text; typed semantic details remain authoritative for client presentation.
