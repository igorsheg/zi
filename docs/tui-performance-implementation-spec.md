...
These are pattern references, not modules to port. OpenCode's client/server projection, large context graph, and Solid component structure do not belong in OpenZi.
...
The handle retains stable header, body, notice, and action-hint owners. A body owner is selected only by the generic body primitive and updates in place while that primitive remains unchanged. Replacing a body subtree clears native selection first. Hidden previews short-circuit before line splitting or wrapping; visible compact previews retain at most 12 rows and detailed previews at most 200 visual rows through directional cell-aware head, tail, or edge admission. The mode-owned `app.tools.expand` binding updates retained handles without replacing roots. One transcript-owned renderer live request refreshes inline timing only while a running tool intersects the viewport; views and tools own no timers. Foreground shell action hints derive from the authoritative session task and semantic keybinding owner, not a tool-name branch. See `docs/tool-presentation-implementation-spec.md`.
...
- `OPENZI_SHOW_TTFD=1`: mount OpenTUI's `TimeToFirstDrawRenderable` in a small diagnostic overlay;
- `OPENZI_TUI_STATS=1`: create the renderer with `gatherStats: true`, retain at most 300 samples, and show OpenTUI's debug statistics overlay;
- `OPENZI_TUI_MEMORY=1`: show an owner-composed memory snapshot covering process memory, session payload, renderer resources, and listeners.
...
`AgentSession` exposes a readonly memory diagnostic containing committed message count and serialized UTF-8 bytes, current streaming bytes, queued input count and bytes, and subscriber count. Committed accounting initializes lazily and advances on the append-only `message_end` path; a replacement or shrink invalidates it for recomputation. Message byte values describe JSON payload size, not JavaScript engine heap allocation.
...
When `OPENZI_TUI_MEMORY=1`, the existing diagnostic overlay captures at most once every three seconds from renderer frame events. It introduces no timer, does not keep an idle renderer live, and retains only the displayed snapshot. Exact Zig allocator usage is not inferred from RSS; renderable and buffer counts are the supported native-resource proxies until OpenTUI exposes allocator statistics.
