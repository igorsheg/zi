...
zi.command({
  ...
  handler = function(_, ctx)
    ...
    if not text then
      ui_toast(ctx, "No assistant message found on the current branch", { kind = "error", lifetime = "until_input" })
      return
    end

    local ok, edited_or_err = pcall(edit_with_editor, quote_text(text), ctx)
    if not ok then
      ui_toast(ctx, tostring(edited_or_err), { kind = "error", lifetime = "until_input" })
      return
    end

    ctx.editor.set_text(edited_or_err)
    ui_toast(ctx, "Loaded edited quoted assistant text into the editor", { kind = "info", lifetime = "until_input" })
  end,
})
