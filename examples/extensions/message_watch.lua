...
zi.on("message", function(event, ctx)
  ...
  if message.role == "user" and message.text then
    ui_toast(ctx, "Last user message: " .. string.sub(message.text, 1, 80))
  elseif message.role == "assistant" and message.text then
    ui_toast(ctx, "Assistant replied: " .. string.sub(message.text, 1, 80))
  elseif message.role == "tool_call" then
    ui_toast(ctx, "Tool requested: " .. tostring(message.tool_name or "unknown"))
  elseif message.role == "tool_result" then
    ui_toast(ctx, "Tool finished: " .. tostring(message.tool_name or "unknown"))
  end
end)
