...
zi.define.event("session_start", function(ctx, _)
  if ctx.ui and ctx.ui.view.set then
    ui_status_spec(ctx, { id = "session", text = "Zi" })
  end
end)

zi.define.event("agent_start", function(ctx, _)
  if ctx.ui and ctx.ui.view.set then
    ui_status_spec(ctx, { id = "session", text = "⠋ Zi working" })
  end
end)

zi.define.event("agent_end", function(ctx, _)
  if ctx.ui and ctx.ui.view.set then
    ui_status_spec(ctx, { id = "session", text = "Zi" })
  end
end)
