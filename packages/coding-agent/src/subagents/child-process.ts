...
import { createProcessScope, type ProcessScope } from "../extensions/process-scope.js"
...
export class ChildZiProcess {
  ...
  readonly #scopeRefresh: ReturnType<typeof setInterval>
  ...
  constructor(options: ChildZiProcessOptions) {
    ...
    this.#processScope = createChildProcessScope(this.#child)
    this.#scopeRefresh = setInterval(() => {
      try {
        if (this.#processScope.refresh().type === "overflow") {
          this.#processScope.terminate()
          this.#fail(new Error("Subagent process scope exceeded tracked descendant capacity"))
        }
      } catch (cause) {
        this.#fail(cause)
      }
  ...
  async start(): Promise<void> {
    const reached = await settleWithin(this.#ready.promise, rpcReadyTimeoutMs)
    if (!reached) {
      const error = new Error(`Subagent ${this.agentId} did not become ready within ${rpcReadyTimeoutMs}ms`)
      this.#fail(error)
      throw error
    }
  ...
  async spawnAdmit(prompt: string): Promise<void> {
    const state = this.#state
    if (state.type !== "idle") throw new Error(`Subagent ${this.agentId} cannot spawn-admit while ${state.type}`)
  ...
  async sendFollowUp(text: string): Promise<void> {
    ...
    if (state.type !== "idle" && state.type !== "running" && state.type !== "interrupting") {
      throw new Error(`Subagent ${this.agentId} cannot accept send while ${state.type}`)
    }
  ...
  async continueWith(text: string): Promise<void> {
    ...
    if (state.type !== "running" && state.type !== "interrupting") {
      throw new Error(`Subagent ${this.agentId} cannot continue while ${state.type}`)
    }
    this.#beginCycleAdmission()
    try {
      await this.request("session.prompt", { delivery: "continue", text })
  ...
  async interrupt(): Promise<"interrupted" | "already_idle"> {
    ...
    if (state.type !== "running" && state.type !== "spawn_admitting") {
      throw new Error(`Subagent ${this.agentId} cannot interrupt while ${state.type}`)
    }
  ...
  async close(reason = "close", graceMs = rpcCloseGraceMs, forceMs = rpcCloseForceMs): Promise<void> {
    ...
    this.#rejectPending(new Error(`Subagent ${this.agentId} is closing`))
    ...
    clearInterval(this.#scopeRefresh)
    this.#processScope.terminate()
    await settleWithin(
      Promise.allSettled([this.#stdoutSettlement, this.#stderrSettlement]).then(() => undefined),
      1_000
    )
    this.#processScope.dispose()
    if (exitCode === timeoutValue) {
      this.#transition({
        type: "exited",
        outcome: { type: "killed", message: `Subagent ${this.agentId} did not exit within close bounds` }
      })
      return
    }
  ...
  request(method: string, params?: Readonly<Record<string, unknown>>): Promise<unknown> {
    if (this.#state.type === "closing" || this.#state.type === "exited") {
      return Promise.reject(new Error(`Subagent ${this.agentId} is ${this.#state.type}`))
    }
    ...
    if (this.#pending.size >= maxRpcPendingRequests) {
      return Promise.reject(new Error(`Subagent ${this.agentId} has too many pending RPC requests`))
    }
  ...
  async #readCompletion(workCycle: number): Promise<SubagentCompletion> {
    ...
    if (!latest) {
      return baseCompletion(this.agentId, workCycle, durationMs, "failed", "", "missing_assistant")
    }
    const clipped = clipUtf8(latest.text, maxCompletionBytes)
    if (latest.stopReason === "aborted") {
      return {
        ...baseCompletion(this.agentId, workCycle, durationMs, "cancelled", clipped.text),
    ...
    if (latest.stopReason === "error") {
      return {
        ...baseCompletion(this.agentId, workCycle, durationMs, "failed", clipped.text, "provider_error"),
    ...
    if (latest.stopReason === "toolUse") {
      return baseCompletion(this.agentId, workCycle, durationMs, "failed", clipped.text, "missing_final_answer")
    }
    if (latest.stopReason === "pending") {
      return baseCompletion(this.agentId, workCycle, durationMs, "failed", clipped.text, "incomplete_final_answer")
    }
    return {
      ...baseCompletion(this.agentId, workCycle, durationMs, "completed", clipped.text),
      originalBytes: clipped.originalBytes,
      omittedBytes: clipped.omittedBytes,
...
function createChildProcessScope(child: Bun.Subprocess<"pipe", "pipe", "pipe">): ProcessScope {
  try {
    return createProcessScope(child.pid)
  } catch (cause) {
    try {
      child.kill()
    } catch {
