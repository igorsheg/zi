...
export function isSubagentToolDetails(value: unknown): value is SubagentToolDetails {
  ...
  switch (value.operation) {
    ...
    case "list":
      return isAgentArray(value.agents) && isStatusNames(value.agents, value.workingNames, value.readyNames)
    default:
      return false
  }
}
...
function isNameArray(value: unknown): value is readonly string[] {
  return (
    Array.isArray(value) && value.length <= 32 && value.every(isSubagentName) && new Set(value).size === value.length
  )
}
