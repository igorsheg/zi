import {
  ...
  type SubagentToolProfileDetails
...
export function projectSubagent(source: ToolPresentationSource): ToolPresentation {
  ...
  switch (operation) {
    case "profiles":
      return profilesPresentation(source, details, fallbackText)
    case "spawn": {
...
function profilesPresentation(
  source: ToolPresentationSource,
  details: SubagentToolDetails | undefined,
  fallbackText: string
): ToolPresentation {
  const profiles = details?.operation === "profiles" ? details.profiles : []
  const count = profiles.length
  const body = profiles.length > 0 ? profiles.map(profileLine).join("\n") : details ? "" : fallbackText
        ...
        ? [
            {
              ...
              text: `${omittedBytes} profile-description bytes omitted`
            }
          ]
        : [],
    preview: {
      compact: body ? { type: "head", rows: Math.min(Math.max(count, 1), 6) } : { type: "hidden" },
...
function profileLine(profile: SubagentToolProfileDetails): string {
  const description = normalizeToolText(profile.description)
  return `${agentNameLabel(profile.name)}${description ? ` — ${summary(description, resultSummaryScalars)}` : ""}`
}
...
function failedPresentation(
  ...
  const subject =
    operation === "profiles"
      ? { type: "text" as const, text: "Profiles" }
      : operation === "spawn"
