  ...
  switch (operation) {
    case "spawn": {
      ...
      return detailOnlyPresentation({
        ...
        subject: agentSubject(agent, stringValue(args?.type)),
        details: prompt ? [summary(prompt, taskSummaryScalars)] : [],
        body: prompt,
        notices: agentNotices(agent),
    ...
    case "send": {
      ...
      return detailOnlyPresentation({
        ...
        notices: agentNotices(agent, stringValue(args?.agent_id)),
        fallbackText: details ? "" : fallbackText,
        timing: "duration"
      })
    }
...
function waitPresentation(
  ...
  const requestedIds = stringArray(args?.agent_ids)
  const count = agents.length || requestedIds.length
  const subject =
    agents.length === 1
      ? agentSubject(agents[0])
      : { type: "text" as const, text: count === 1 ? "Agent" : `${count || "…"} agents` }
  const body = agents.length > 0 ? agents.map(waitLine).join("\n") : details ? "" : fallbackText
  return {
    header: {
    ...
    notices: agentIdNotice(
      agents.map(agent => agent.agentId).length > 0 ? agents.map(agent => agent.agentId) : requestedIds
    ),
    preview: {
      compact: body ? { type: "head", rows: 6 } : { type: "hidden" },
...
function listPresentation(
  ...
  const working = list?.workingAgentIds.length ?? 0
  const ready = list?.readyAgentIds.length ?? 0
  const counts = [
    ...(working > 0 ? [`${working} working`] : []),
    ...(ready > 0 ? [`${ready} ready`] : []),
    ...(agents.length === 0 && source.status === "done" ? ["none"] : [])
  ]
  const readyAgentIds = new Set(list?.readyAgentIds ?? [])
  const body =
    agents.length > 0 ? agents.map(agent => listLine(agent, readyAgentIds)).join("\n") : details ? "" : fallbackText
  return {
    header: { label: source.status === "done" ? "Checked agents" : "Check agents", details: counts },
    ...(body ? { body: { type: "text" as const, text: boundHead(body), tone: "muted" as const } } : {}),
    notices: agentIdNotice(agents.map(agent => agent.agentId)),
    preview: {
      compact: { type: "hidden" },
      detailed: body ? { type: "head", rows: maxExpandedToolRows } : { type: "hidden" }
    },
...
function failedPresentation(
  ...
  const subject =
    operation === "spawn"
      ? agentSubject(undefined, stringValue(args?.type))
      : operation === "wait"
        ? { type: "text" as const, text: `${stringArray(args?.agent_ids).length || "…"} agents` }
        : operation === "list"
          ? undefined
...
function agentSubject(
  ...
  return { type: "text", text: definitionLabel(agent?.definitionName ?? fallbackType ?? "Agent") }
}

function agentNotices(agent: SubagentToolAgentDetails | undefined, fallbackId?: string): readonly ToolNotice[] {
  return agentIdNotice(agent ? [agent.agentId] : fallbackId ? [fallbackId] : [])
}

function agentIdNotice(ids: readonly string[]): readonly ToolNotice[] {
  ...
  return [
    {
      ...
      tone: "muted",
      visibility: "detailed",
      text: boundInline(`${ids.length === 1 ? "Agent id" : "Agent ids"}: ${ids.join(", ")}`)
    }
  ]
}

function waitLine(agent: SubagentToolAgentDetails): string {
  const label = definitionLabel(agent.definitionName)
  const completion = agent.completion
  if (!completion || (agent.workCycle !== undefined && completion.workCycle !== agent.workCycle)) {
    return `${label} ${lifecycleLabel(agent.lifecycle)}`
  }
...
function listLine(agent: SubagentToolAgentDetails, readyAgentIds: ReadonlySet<string>): string {
  const lifecycle = lifecycleLabel(agent.lifecycle)
  const state = readyAgentIds.has(agent.agentId)
    ? agent.lifecycle === "idle" || agent.lifecycle === "exited"
      ? "result ready"
      : `${lifecycle} · result ready`
    : lifecycle
  return `${definitionLabel(agent.definitionName)} — ${state}`
}
