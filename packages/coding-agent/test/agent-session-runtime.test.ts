...
test("session catalogs stay globally serialized and restart for a replaced runtime", async () => {
  const root = await mkdtemp(join(tmpdir(), "openzi-session-runtime-list-generation-"))
  const models = createModels()
...
test("session catalog browsing remains read-only while a provider run is active", async () => {
  const root = await mkdtemp(join(tmpdir(), "openzi-session-runtime-list-active-"))
  const models = createModels()
...
test("session runtime rebuilds every cwd-bound service from the resumed header", async () => {
  const root = await mkdtemp(join(tmpdir(), "openzi-session-runtime-switch-"))
  ...
  try {
    ...
    expect(runtime.services.paths.projectDir).toBe(join(target.services.paths.cwd, ".openzi"))
...
test("cancelling an in-flight replacement keeps the current session and removes its new journal", async () => {
  const root = await mkdtemp(join(tmpdir(), "openzi-session-runtime-cancel-"))
