...
test("code-mode protocol decodes fragmented and coalesced frames", () => {
  ...
  const first = encodeCodeModeFrame({ version: codeModeProtocolVersion, type: "ready" })
  ...
  expect(decoder.push(input.subarray(3, first.byteLength + 2))).toEqual([
    { version: codeModeProtocolVersion, type: "ready" }
  ])
  expect(decoder.push(input.subarray(first.byteLength + 2))).toEqual([
    { version: codeModeProtocolVersion, type: "completed", result: { ok: true }, logs: [] }
  ])
  decoder.end()
})
...
test("code-mode protocol bounds queued host output", async () => {
  ...
  for (let index = 0; index < 128; index++) {
    pending.push(writer.send({ version: codeModeProtocolVersion, type: "start", code: "async () => {}", tools: [] }))
  }
  const overflow = writer.send({ version: codeModeProtocolVersion, type: "start", code: "async () => {}", tools: [] })
...
test("code-mode protocol fails closed on partial, oversized, and structurally unbounded input", () => {
  ...
  partial.push(encodeCodeModeFrame({ version: codeModeProtocolVersion, type: "ready" }).subarray(0, 6))
