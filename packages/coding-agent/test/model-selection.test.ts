...
test("model and thinking selections persist a coherent canonical transition", async () => {
  const root = await mkdtemp(join(tmpdir(), "zi-thinking-selection-"))
...
test("thinking mutations report an effective project override without false events", async () => {
  const root = await mkdtemp(join(tmpdir(), "zi-thinking-shadow-"))
  const cwd = join(root, "project")
  ...
  await writeFile(join(cwd, ".zi", "settings.json"), JSON.stringify({ defaultThinkingLevel: "high" }))
  const models = createModels()
