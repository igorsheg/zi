...
test("project file search roots Git enumeration at ZiPaths.cwd and respects standard ignores", async () => {
  const root = await mkdtemp(join(tmpdir(), "zi-file-search-git-"))
  ...
  const search = new ProjectFileSearch(new ZiPaths(cwd, join(root, "global")))
...
test("non-Git fallback is ignore-aware, includes hidden entries, and never traverses directory symlinks", async () => {
  const root = await mkdtemp(join(tmpdir(), "zi-file-search-walk-"))
...
test("fallback stops a subtree when its ignore policy exceeds the byte budget", async () => {
  const root = await mkdtemp(join(tmpdir(), "zi-file-search-ignore-bound-"))
  const cwd = join(root, "project")
...
test("fallback skips a subtree whose ignore file is unreadable", async () => {
  ...
  const root = await mkdtemp(join(tmpdir(), "zi-file-search-ignore-unreadable-"))
  const cwd = join(root, "project")
  await mkdir(cwd, { recursive: true })
  await writeFile(join(cwd, "secret.ts"), "secret")
...
test("ranking prefers exact path, basename, prefix, and ordered nested segments", async () => {
  const root = await mkdtemp(join(tmpdir(), "zi-file-search-rank-"))
  const cwd = join(root, "project")
...
test("invalid queries fail before I/O and one search operation is single-flight and cancellable", async () => {
  const root = await mkdtemp(join(tmpdir(), "zi-file-search-state-"))
  const cwd = join(root, "project")
  await mkdir(cwd, { recursive: true })
  const search = new ProjectFileSearch(new ZiPaths(cwd, join(root, "global")))
