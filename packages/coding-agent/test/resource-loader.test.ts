...
test("subagent profiles load from global and trusted project resources with project precedence", async () => {
  const root = await mkdtemp(join(tmpdir(), "zi-subagent-profiles-"))
  const paths = new ZiPaths(join(root, "project"), join(root, "global"))
  await mkdir(paths.globalResourceDir("subagents"), { recursive: true })
  await mkdir(paths.projectResourceDir("subagents"), { recursive: true })
  await writeFile(
    join(paths.globalResourceDir("subagents"), "finder.md"),
    "---\ndescription: Global finder\nmodel: anthropic/claude-opus-4-6\nthinking: low\n---\nFind global references."
  )
  await writeFile(
    join(paths.projectResourceDir("subagents"), "finder.md"),
    "---\ndescription: Project finder\nthinking: high\n---\nFind project references."
  )
  await writeFile(
    join(paths.globalResourceDir("subagents"), "reviewer.md"),
    "---\ndescription: Review changes\n---\nReview the implementation."
  )

  const resources = await new ResourceLoader({ paths, project: "trusted" }).load()

  expect(resources.subagentProfiles).toMatchObject([
    {
      name: "finder",
      description: "Project finder",
      instructions: "Find project references.",
      thinking: "high",
      scope: "project"
    },
    { name: "reviewer", description: "Review changes", instructions: "Review the implementation.", scope: "global" }
  ])
  expect(resources.diagnostics).toContainEqual(
    expect.objectContaining({ type: "collision", resource: "subagent-profile", name: "finder" })
  )
})

test("untrusted and invalid subagent profiles are excluded with source-attributed diagnostics", async () => {
  const root = await mkdtemp(join(tmpdir(), "zi-subagent-profile-admission-"))
  const paths = new ZiPaths(join(root, "project"), join(root, "global"))
  await mkdir(paths.globalResourceDir("subagents"), { recursive: true })
  await mkdir(paths.projectResourceDir("subagents"), { recursive: true })
  await writeFile(
    join(paths.projectResourceDir("subagents"), "project-only.md"),
    "---\ndescription: Project only\n---\nDo not load."
  )
  await writeFile(join(paths.globalResourceDir("subagents"), "missing-description.md"), "No description")
  await writeFile(
    join(paths.globalResourceDir("subagents"), "bad-thinking.md"),
    "---\ndescription: Invalid thinking\nthinking: enormous\n---\nBody"
  )
  await writeFile(
    join(paths.globalResourceDir("subagents"), "Bad Name.md"),
    "---\ndescription: Invalid name\n---\nBody"
  )
  await writeFile(
    join(paths.globalResourceDir("subagents"), "valid.md"),
    "---\ndescription: Valid profile\n---\nValid body"
  )

  const resources = await new ResourceLoader({ paths, project: "untrusted" }).load()

  expect(resources.subagentProfiles.map(profile => profile.name)).toEqual(["valid"])
  expect(resources.diagnostics).toEqual(
    expect.arrayContaining([
      expect.objectContaining({
        resource: "subagent-profile",
        path: expect.stringContaining("missing-description.md")
      }),
      expect.objectContaining({ resource: "subagent-profile", path: expect.stringContaining("bad-thinking.md") }),
      expect.objectContaining({ resource: "subagent-profile", path: expect.stringContaining("Bad Name.md") })
    ])
  )
  expect(
    resources.diagnostics.some(diagnostic => "path" in diagnostic && diagnostic.path.includes("project-only.md"))
  ).toBe(false)
})
