...
test("the OpenTUI prompt drives a complete tool turn", async () => {
  ...
  try {
    ...
    const writeRow = frameRows.findIndex(row => row.includes("◆ Write answer.txt"))
    const resultRow = frameRows.findIndex(row => row.includes("Checking the result."))
    expect(writeRow).toBeGreaterThanOrEqual(0)
    expect(resultRow).toBeGreaterThan(writeRow)
    expect(frameRows.slice(writeRow, resultRow).join("\n")).not.toContain("╭───")
