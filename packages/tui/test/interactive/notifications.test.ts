...
test("interactive notifications anchor to the transcript above the composer", async () => {
  ...
  try {
    ...
    expect(rows[1]).toEndWith(" Ready INFO  ")
    expect(rows[2]).toEndWith(" Notifications ❰❰  ")
    expect(rows[3]?.trimEnd()).toBe("")
    expect(rows[4]).toStartWith("╭─")
    expect(rows[7]).toContain("/work")
  } finally {
    session.dispose()
    setup.destroy()
  }
