...
import { createModels, createTestAgentRuntime, fauxProvider } from "@openzi/coding-agent/testing"
...
test("global settings explain an effective project override", async () => {
  const root = await mkdtemp(join(tmpdir(), "openzi-settings-picker-shadow-"))
  ...
  await writeFile(join(cwd, ".openzi", "settings.json"), JSON.stringify({ steeringMode: "all" }))
...
test("invalid project settings refuse picker writes without changing live behavior", async () => {
  const root = await mkdtemp(join(tmpdir(), "openzi-settings-picker-invalid-"))
  const cwd = join(root, "project")
  const agentDir = join(root, "global")
  const projectFile = join(cwd, ".openzi", "settings.json")
...
async function createSettingsFixture(
  ...
  const root = await mkdtemp(join(tmpdir(), "openzi-settings-picker-"))
