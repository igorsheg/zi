...
async function buildLocal(): Promise<void> {
  ...
  const executable = join(outputDirectory, `zi${extension}`)
  const pending = join(outputDirectory, `.zi-${process.pid}${extension}`)
  const version = process.env.ZI_BUILD_VERSION ?? (await workingTreeVersion(root))
