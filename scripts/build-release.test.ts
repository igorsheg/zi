...
test("release options use bounded environment inputs", () => {
  expect(
    parseReleaseBuildOptions([], { OPENZI_RELEASE_VERSION: "2.0.0", OPENZI_RELEASE_TARGET: "darwin-arm64" })
  ).toEqual({ version: "2.0.0", target: "darwin-arm64" })
  expect(() => parseReleaseBuildOptions([], {})).toThrow("Release version is required")
  expect(() => parseReleaseBuildOptions(["--other"], { OPENZI_RELEASE_VERSION: "1.0.0" })).toThrow(
...
test("release archive names are stable", () => {
  expect(releaseArchiveName({ version: "1.2.3", target: "windows-x64" })).toBe("openzi-1.2.3-windows-x64.tar.gz")
})
