...
export function parseReleaseBuildOptions(
  ...
  let version = env.OPENZI_RELEASE_VERSION
  let target = env.OPENZI_RELEASE_TARGET
  ...
  if (!version) throw new Error("Release version is required through --version or OPENZI_RELEASE_VERSION")
...
export function releaseArchiveName(options: ReleaseBuildOptions): string {
  return `openzi-${options.version}-${options.target}.tar.gz`
}
...
async function buildRelease(options: ReleaseBuildOptions): Promise<void> {
  ...
  const packageName = `openzi-${options.version}-${options.target}`
  const packageDirectory = join(outputDirectory, packageName)
  const executableName = options.target.startsWith("windows-") ? "openzi.exe" : "openzi"
  ...
  try {
    await compileOpenZi({ outfile: executable, version: options.version })
