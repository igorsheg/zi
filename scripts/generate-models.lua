...
local function find_model(list, provider, id)
	for _, m in ipairs(list) do
		if m.provider == provider and m.id == id then
			return m
		end
	end
	return nil
	...
	for _, a in ipairs(openai_adds) do
		if not find_model(all, "openai", a.id) then
			all[#all + 1] = {
				...
				cost_input = a.ci,
				cost_output = a.co,
				cost_cache_read = a.cr,
				cost_cache_write = a.cw,
				context_window = a.ctx,
				max_tokens = a.max_tok,
			}
		end
	end
	...
	for _, m in ipairs(models) do
		...
		w(
			string.format(
				...
				fmt_num(m.cost_input),
				fmt_num(m.cost_output),
				fmt_num(m.cost_cache_read),
				fmt_num(m.cost_cache_write)
			)
		)
...
local all = {}
for _, m in ipairs(dev_models) do
	all[#all + 1] = m
end
for _, m in ipairs(or_models) do
	all[#all + 1] = m
end
for _, m in ipairs(codex_models) do
	all[#all + 1] = m
end
...
local counts = {}
for _, m in ipairs(all) do
	counts[m.provider] = (counts[m.provider] or 0) + 1
end
local keys = {}
