...
pub fn main(init: std.process.Init) !void {
    ...
    const models_dev_body = try fetch(allocator, init.io, "https://models.dev/api.json");
    ...
    const openrouter_body = try fetch(allocator, init.io, "https://openrouter.ai/api/v1/models");
    std.debug.print("  {d} bytes\n", .{openrouter_body.len});
    ...
    try writeOutput(init.io, output_path, all.items.items);
...
fn writeOutput(io: std.Io, output_path: []const u8, models: []const Model) !void {
    ...
    try w.writeAll("pub const providers = [_]protocol.Provider{\n");
    try writeProviders(w, models);
    try w.writeAll("};\n\n");
    ...
    for (models) |model| {
        ...
        try w.print("        .api = protocol.KnownApi.{s},\n", .{model.api});
        try w.print("        .provider = protocol.KnownProvider.{s},\n", .{model.provider});
...
fn writeProviders(w: *std.Io.Writer, models: []const Model) !void {
    var previous: ?[]const u8 = null;
    for (models) |model| {
        if (previous) |provider| {
            if (std.mem.eql(u8, provider, model.provider)) continue;
        }
        try w.print("    protocol.KnownProvider.{s},\n", .{model.provider});
