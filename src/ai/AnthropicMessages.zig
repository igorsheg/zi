...
const Body = @import("AnthropicMessagesBody.zig");
const Events = @import("AnthropicMessagesEvents.zig");
...
/// Generic Anthropic Messages adapter. Recipe/config owners inject all
/// credentials and resolved first-party or compatible policy.
pub const AnthropicMessages = struct {
    pub const Config = ConfigData;
    pub const ThinkingMode = Body.ThinkingMode;

    transport: Transport.Transport,
    config: Config,

    pub fn init(transport: Transport.Transport, config: Config) AnthropicMessages {
        return .{ .transport = transport, .config = config };
    }

    pub fn provider(self: *AnthropicMessages) Provider.Provider {
...
fn validateConfig(config: ConfigData) error{InvalidRequest}!void {
    if (config.provider_id.len == 0 or !std.unicode.utf8ValidateSlice(config.provider_id) or
        !validEndpoint(config.endpoint) or !validVersion(config.version) or
        config.maximum_api_key_bytes == 0 or
    ...
    if (config.events.max_tracked_blocks == 0 or
        config.events.max_tracked_blocks > Events.maximum_tracked_blocks or
    ...
    var header_bytes: usize = "anthropic-version".len + config.version.len +
        "Accept".len + "text/event-stream".len +
        "Content-Type".len + "application/json".len;
    if (config.api_key) |api_key| {
        header_bytes = std.math.add(usize, header_bytes, "x-api-key".len + api_key.len) catch
            return error.InvalidRequest;
    }
...
const AttemptFactory = struct {
    ...
    pub fn create(
        ...
        const count = 3 + self.extra_headers.len + @as(usize, @intFromBool(self.api_key != null));
        const headers = try allocator.alloc(Transport.Header, count);
        ...
        headers[next] = .{ .name = "anthropic-version", .value = self.version };
        next += 1;
        ...
        state.* = .{ .parser = parser, .headers = headers };
...
const TestTransport = struct {
    ...
    pub fn ssePost(
        ...
        self.calls += 1;
        self.valid = self.valid and std.mem.eql(u8, request.url, "https://messages.test/v1/messages");
        self.valid = self.valid and request.headers.len == 5;
        self.valid = self.valid and request.headers[0].isPrivileged();
        self.valid = self.valid and std.mem.eql(u8, request.headers[0].name, "x-api-key");
        self.valid = self.valid and std.mem.eql(u8, request.headers[0].value, "secret");
        self.valid = self.valid and std.mem.eql(u8, request.headers[1].value, default_version);
        self.valid = self.valid and std.mem.eql(u8, request.headers[4].name, "X-Client");
        self.valid = self.valid and std.mem.find(u8, request.json_body, "\"max_tokens\":32000") != null;
        if (self.incomplete_once and self.calls == 1) {
            try sink.emit(.{
                .data = "{\"type\":\"message_start\",\"message\":{" ++
                    "\"id\":\"old\",\"model\":\"old\",\"usage\":{\"input_tokens\":4}}}",
            });
            return .{ .status = 200, .outcome = .completed };
        }
        try sink.emit(.{
            .data = "{\"type\":\"message_start\",\"message\":{" ++
                "\"id\":\"new\",\"model\":\"served\",\"usage\":{\"input_tokens\":2}}}",
        });
        try sink.emit(.{
        ...
        switch (event) {
            ...
            .done => |done| {
                ...
                self.response_ok = done.response.id != null and
                    ...
                    done.response.model != null and
                    std.mem.eql(u8, done.response.model.?, "served") and
...
fn testConfig() AnthropicMessages.Config {
    return .{
        .provider_id = "anthropic-compatible",
        .endpoint = "https://messages.test/v1/messages",
        .api_key = "secret",
        .extra_headers = &.{.{ .name = "X-Client", .value = "Zi" }},
        .retry = .{ .policy = .{ .max_attempts = 2, .base_delay_ms = 0 } },
    };
}

test "Messages adapter retries incomplete streams with fresh identity and headers" {
    var fake: TestTransport = .{ .incomplete_once = true };
    var adapter = AnthropicMessages.init(Transport.Transport.from(&fake), testConfig());
    try std.testing.expectEqualStrings("anthropic-compatible", adapter.provider().id);
...
test "Messages config rejects key injection and fixed header replacement" {
    var fake: TestTransport = .{};
    var config = testConfig();
    config.api_key = "bad\nkey";
    var adapter = AnthropicMessages.init(Transport.Transport.from(&fake), config);
    var collector: TestCollector = .{};
    ...
    config.api_key = null;
    config.extra_headers = &.{.{ .name = "anthropic-version", .value = "other" }};
    adapter = AnthropicMessages.init(Transport.Transport.from(&fake), config);
    try std.testing.expectError(error.InvalidRequest, adapter.provider().stream(
        std.testing.allocator,
        std.testing.io,
        request,
...
test "Messages attempt construction releases every partial allocation" {
...
test "Messages config validates endpoint authority and date version" {
    var fake: TestTransport = .{};
    const request: Provider.Request = .{
        .model = "model",
        .context = .{ .system_prompt = "", .items = &.{}, .tools = &.{} },
    ...
    for ([_][]const u8{
        "https://:",
        "https://user@host/messages",
        "https://host/messages#fragment",
        "https://host:0/messages",
        "https://[::1/messages",
    }) |endpoint| {
        var config = testConfig();
    ...
    for ([_][]const u8{ "", "2023-6-01", "2023-13-01", "2023-02-29", "2023-06-00", "future\tversion" }) |version| {
        var config = testConfig();
