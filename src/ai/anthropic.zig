...
pub const AnthropicProvider = struct {
    ...
    fn streamImpl(self: *AnthropicProvider, allocator: std.mem.Allocator, model: protocol.Model, context: protocol.Context, options: protocol.StreamOptions, reasoning: ?protocol.ThinkingLevel, thinking_budgets: ?protocol.ThinkingBudgets, sink: ai_provider.StreamEventSink) void {
        ...
        if (state.message.stop_reason == .aborted) {
            ...
            state.message.stop_reason = switch (sr) {
                .end_turn => .stop,
                .max_tokens => .length,
                .tool_use => .toolUse,
                else => .stop,
            };
            sink.emit(.{ .done = .{ .reason = done_reason, .message = state.message } });
        } else {
            sink.emit(.{ .done = .{ .reason = .stop, .message = state.message } });
        }
    }
...
fn handleSseEvent(evt: sse.SseEvent, state: *StreamState, sink: ai_provider.StreamEventSink) !void {
    const data = evt.data;
    ...
    if (std.mem.eql(u8, event_type, "message_start")) {
        ...
        if (std.mem.eql(u8, block_type, "text")) {
            ...
            sink.emit(.{ .text_start = .{ .content_index = state.content_blocks.items.len - 1 } });
        ...
        if (std.mem.eql(u8, delta_type, "text_delta")) {
            ...
            if (block.tool_call) |*tc| {
                ...
                sink.emit(.{ .toolcall_delta = .{ .content_index = index, .delta = json_delta } });
            }
        }
        ...
        switch (block.block_type) {
            .text => {
                sink.emit(.{ .text_end = .{ .content_index = index, .content = block.text.items } });
            },
            .thinking => {
                sink.emit(.{ .thinking_end = .{ .content_index = index, .content = block.thinking.items } });
            },
            .tool_call => {
                if (block.tool_call) |tc| {
                    var final_tc = tc;
                    ...
                    sink.emit(.{ .toolcall_end = .{ .content_index = index, .tool_call = final_tc } });
                }
            },
        }
...
test "Anthropic SSE usage keeps cache tokens in total_tokens" {
    ...
    try testing.expectEqual(@as(u64, 800), state.message.usage.cache_read);
    try testing.expectEqual(@as(u64, 400), state.message.usage.cache_write);
    ...
    try testing.expectEqual(@as(u64, 900), state.message.usage.cache_read);
    try testing.expectEqual(@as(u64, 400), state.message.usage.cache_write);
    try testing.expectEqual(@as(u64, 2525), state.message.usage.total_tokens);
}
