...
pub const Runtime = struct {
    ...
    model_registry: *ai.model_registry.ModelRegistry,
    ...
    pub fn init(allocator: std.mem.Allocator) std.mem.Allocator.Error!InitResult {
        ...
        const model_registry = try allocator.create(ai.model_registry.ModelRegistry);
        errdefer allocator.destroy(model_registry);
        model_registry.* = ai.model_registry.ModelRegistry.init(allocator, auth_storage, custom_models) catch |err| switch (err) {
            error.OutOfMemory => return error.OutOfMemory,
        };
