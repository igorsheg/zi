...
test "project trust store resolves nearest decisions and supports revocation" {
    ...
    try std.testing.expectEqual(Decision.trusted, inherited.nearest(&child).?.decision);
    inherited.deinit();
    ...
    try std.testing.expectEqual(Decision.untrusted, overridden.nearest(&child).?.decision);
    overridden.deinit();
...
test "project trust store distinguishes failed and indeterminate commits" {
    ...
    try std.testing.expectEqual(Decision.untrusted, original.exact(&identity).?.decision);
    original.deinit();
    ...
    try std.testing.expectEqual(Decision.trusted, published.exact(&identity).?.decision);
}
