...
test "ai completion worker publishes faux provider result" {
    const testing = std.testing;
    const allocator = testing.allocator;

    var faux_provider = ai.faux.FauxProvider.init(allocator);
    defer faux_provider.deinit();
    const content = [_]ai.protocol.AssistantMessage.AssistantContentBlock{ai.faux.fauxText("summary text")};
    const response = ai.faux.fauxAssistantMessage(allocator, &content, .stop);
    defer allocator.free(response.content);
    faux_provider.setResponses(&.{response});
    ...
    var result = sink.result orelse return error.MissingAiCompleteResult;
    defer result.deinit(allocator);
    try testing.expect(result == .ai_complete);
    try testing.expect(result.ai_complete == .completed);
    try testing.expectEqualStrings("summary text", result.ai_complete.completed.text);
    try testing.expectEqual(@as(usize, 1), faux_provider.call_count);
}
