...
pub fn get(name: []const u8) ?[]const u8 {
    if (process_environment) |environ_map| {
        if (environ_map.get(name)) |value| return value;
        return null;
    }

    // Tests and a few standalone helpers may call env.get without going
    // through main(). Keep a narrow fallback rather than forcing every unit
    // test to construct a process.Init.
