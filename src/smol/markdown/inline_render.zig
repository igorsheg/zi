// Ported from vercel-labs/fx src/core/agent/presentation/inline_render.zig at commit 5ed3be1.
        ...
        if (c == '[') {
            if (footnotes) |sink| {
                if (parseFootnoteReference(text, i)) |reference| {
                    const number = try sink.register(alloc, sink.ctx, reference.label);
...
fn parseBareUrl(
    ...
    while (end < text.len and !isBareUrlTerminator(
        text,
        end,
        in_bold,
        in_italic,
...
fn malformedInlineLinkCandidateEnd(text: []const u8, start: usize) ?usize {
    ...
    while (candidate_end < text.len and
        text[candidate_end] != ')' and text[candidate_end] != '\n') : (candidate_end += 1)
    {}
...
test "writeInline renders bold, italic, and code" {
    var out: std.ArrayList(u8) = .empty;
    defer out.deinit(std.testing.allocator);
    var link_id: u32 = 0;
    try writeInline(std.testing.allocator, "**bold** *it* `code`", &out, false, null, &link_id);
    const text = try std.testing.allocator.dupe(u8, out.items);
    defer std.testing.allocator.free(text);
    try std.testing.expect(std.mem.indexOf(u8, text, ansi.bold_open) != null);
    try std.testing.expect(std.mem.indexOf(u8, text, "bold") != null);
    try std.testing.expect(std.mem.indexOf(u8, text, ansi.italic_open) != null);
    try std.testing.expect(std.mem.indexOf(u8, text, ansi.inline_code_open) != null);
}

test "writeInlineNoBold strips bold markers" {
    var out: std.ArrayList(u8) = .empty;
    defer out.deinit(std.testing.allocator);
    var link_id: u32 = 0;
    try writeInlineNoBold(std.testing.allocator, "**bold** plain", &out, false, null, &link_id);
    try std.testing.expect(std.mem.indexOf(u8, out.items, "bold") != null);
    try std.testing.expect(std.mem.indexOf(u8, out.items, "**") == null);
}

test "writeInline emits OSC 8 links" {
    var out: std.ArrayList(u8) = .empty;
    defer out.deinit(std.testing.allocator);
    var link_id: u32 = 7;
    try writeInline(std.testing.allocator, "[label](https://example.com)", &out, false, null, &link_id);
    try std.testing.expectEqual(@as(u32, 8), link_id);
    try std.testing.expect(std.mem.indexOf(u8, out.items, "]8;id=fx-7;") != null);
    try std.testing.expect(std.mem.indexOf(u8, out.items, "https://example.com") != null);
    try std.testing.expect(std.mem.indexOf(u8, out.items, "label") != null);
}

test "angle autolink becomes a mailto link" {
    var out: std.ArrayList(u8) = .empty;
    defer out.deinit(std.testing.allocator);
    var link_id: u32 = 0;
    try writeInline(std.testing.allocator, "<a@b.co>", &out, false, null, &link_id);
    try std.testing.expect(std.mem.indexOf(u8, out.items, "mailto:a@b.co") != null);
}

test "bare url is linked and trailing punctuation kept literal" {
    var out: std.ArrayList(u8) = .empty;
    defer out.deinit(std.testing.allocator);
    var link_id: u32 = 0;
    try writeInline(std.testing.allocator, "see https://example.com/x.", &out, false, null, &link_id);
    try std.testing.expect(std.mem.indexOf(u8, out.items, "https://example.com/x") != null);
}

test "footnote reference registers with sink" {
    const Sink = struct {
        var registered: ?[]const u8 = null;
        fn register(_: std.mem.Allocator, _: *anyopaque, label: []const u8) anyerror!usize {
            registered = label;
            return 3;
        }
    };
    var sink_state: u8 = 0;
    const sink: payload.FootnoteSink = .{ .ctx = @ptrCast(&sink_state), .register = Sink.register };
    var out: std.ArrayList(u8) = .empty;
    defer out.deinit(std.testing.allocator);
    var link_id: u32 = 0;
    try writeInline(std.testing.allocator, "note[^1]", &out, false, &sink, &link_id);
    try std.testing.expectEqualStrings("1", Sink.registered.?);
