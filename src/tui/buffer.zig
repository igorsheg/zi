...
test "Buffer init and clear" {
    var buf = try Buffer.init(std.testing.allocator, 10, 5);
    defer buf.deinit();

    const c = buf.get(0, 0);
    try std.testing.expect(c.eql(Cell.blank));

    buf.set(3, 2, Cell{ .grapheme = .{ .codepoint = 'X' }, .fg = Color.rgb(255, 0, 0) });
    const c2 = buf.get(3, 2);
    try std.testing.expectEqual(@as(u21, 'X'), c2.grapheme.codepoint);
...
test "Buffer writeStr" {
    ...
    const cols = buf.writeStr(0, 0, "hello", Color.default, Color.default, Attributes.none);
...
test "Buffer writeStr stops at edge" {
    var buf = try Buffer.init(std.testing.allocator, 3, 1);
    defer buf.deinit();

    const cols = buf.writeStr(0, 0, "hello", Color.default, Color.default, Attributes.none);
    try std.testing.expectEqual(@as(u32, 3), cols);
}

test "Region clips writes" {
    var buf = try Buffer.init(std.testing.allocator, 10, 10);
    defer buf.deinit();

    const r = Region{ .buf = &buf, .x = 2, .y = 3, .width = 4, .height = 2 };
    r.set(0, 0, Cell{ .grapheme = .{ .codepoint = 'A' } });

    try std.testing.expectEqual(@as(u21, 'A'), buf.get(2, 3).grapheme.codepoint);

    r.set(5, 0, Cell{ .grapheme = .{ .codepoint = 'B' } });
    try std.testing.expect(buf.get(7, 3).eql(Cell.blank));
}

test "Region sub-region" {
    var buf = try Buffer.init(std.testing.allocator, 20, 20);
    defer buf.deinit();

    const outer = Region{ .buf = &buf, .x = 5, .y = 5, .width = 10, .height = 10 };
    const inner = outer.sub(2, 3, 4, 4);

    try std.testing.expectEqual(@as(u32, 7), inner.x);
    try std.testing.expectEqual(@as(u32, 8), inner.y);
    try std.testing.expectEqual(@as(u32, 4), inner.width);
    try std.testing.expectEqual(@as(u32, 4), inner.height);
}
...
test "Buffer fill" {
    ...
    const c = Cell{ .grapheme = .{ .codepoint = '#' }, .fg = Color.rgb(255, 255, 0) };
    buf.fill(1, 1, 3, 2, c);

    try std.testing.expectEqual(@as(u21, '#'), buf.get(1, 1).grapheme.codepoint);
    try std.testing.expectEqual(@as(u21, '#'), buf.get(3, 2).grapheme.codepoint);
    try std.testing.expect(buf.get(0, 0).eql(Cell.blank));
}

test "GraphemePool add and get" {
...
test "Buffer set out of bounds is no-op" {
    var buf = try Buffer.init(std.testing.allocator, 5, 5);
    defer buf.deinit();
    buf.set(10, 10, Cell{ .grapheme = .{ .codepoint = 'X' } });
}
