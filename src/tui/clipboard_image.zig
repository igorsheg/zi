...
fn readViaMacOs(
    ...
    const mime_type = allocator.dupe(u8, "image/png") catch return error.OutOfMemory;
    return .{ .bytes = bytes, .mime_type = mime_type };
}
...
fn readViaWlPaste(
    ...
    const mime_type = allocator.dupe(u8, base) catch {
        allocator.free(data.stdout);
        return error.OutOfMemory;
    };
    return .{ .bytes = data.stdout, .mime_type = mime_type };
}
