...
const TextAreaLayoutCache = layout_mod.TextAreaLayoutCache;
...
pub const TextAreaView = struct {
    ...
    pub fn init(allocator: Allocator, buffer: *EditBuffer) TextAreaView {
        return .{
            .buffer = buffer,
            .layout = TextAreaLayoutCache.init(allocator),
        };
    }

    pub fn deinit(self: *TextAreaView) void {
        self.layout.deinit();
    }

    pub fn setLayoutConfig(self: *TextAreaView, config: LayoutConfig) void {
    ...
    pub fn setViewportHeight(self: *TextAreaView, height_rows: u32) void {
        self.viewport.height_rows = @max(@as(u32, 1), height_rows);
        self.clampViewport();
    }

    pub fn resetViewport(self: *TextAreaView) void {
        self.viewport.scroll_x = 0;
        self.viewport.scroll_y = 0;
        self.cursor_cache = null;
    }

    pub fn setScrollMargin(self: *TextAreaView, margin: f32) void {
        self.scroll_margin = @max(0.0, @min(0.5, margin));
    }

    pub fn ensureLayout(self: *TextAreaView) void {
        self.layout.ensure(self.buffer, self.layout_config);
        self.clampViewport();
    }

    pub fn invalidateAll(self: *TextAreaView) void {
        self.layout.invalidateAll();
        self.cursor_cache = null;
    }

    pub fn visibleLines(self: *TextAreaView) []const VirtualLine {
        self.ensureLayout();
    ...
    pub fn totalVisualLineCount(self: *TextAreaView) u32 {
        self.ensureLayout();
        return @intCast(self.layout.lines().len);
    }

    pub fn viewportState(self: *const TextAreaView) Viewport {
        return self.viewport;
    }

    pub fn viewportHeight(self: *const TextAreaView) u32 {
        return self.viewport.height_rows;
    }

    pub fn clearDesiredVisualColumn(self: *TextAreaView) void {
        self.desired_visual_col = null;
    }

    pub fn visualCursor(self: *TextAreaView) ?VisualCursor {
        self.ensureLayout();
    ...
    pub fn logicalToVisual(self: *TextAreaView, logical: LogicalCursor) ?VisualCursor {
        self.ensureLayout();
    ...
    pub fn visualToLogical(self: *TextAreaView, row: u32, col: u32) ?LogicalCursor {
        self.ensureLayout();
    ...
    pub fn moveUpVisual(self: *TextAreaView) void {
        self.moveVisual(-1);
    }

    pub fn moveDownVisual(self: *TextAreaView) void {
        self.moveVisual(1);
    }

    pub fn ensureCursorVisible(self: *TextAreaView) void {
        self.ensureLayout();
    ...
    pub fn isCursorOnFirstVisualLine(self: *TextAreaView) bool {
        self.ensureLayout();
        const resolved = self.resolveCursor() orelse return true;
        return resolved.absolute_row == 0;
    }

    pub fn isCursorOnLastVisualLine(self: *TextAreaView) bool {
        self.ensureLayout();
        const resolved = self.resolveCursor() orelse return true;
        return resolved.absolute_row + 1 >= self.layout.lines().len;
    }

    fn moveVisual(self: *TextAreaView, direction: i32) void {
        self.ensureLayout();
        const resolved = self.resolveCursor() orelse return;
    ...
    fn resolveCursor(self: *TextAreaView) ?ResolvedCursor {
    ...
    fn logicalToResolved(self: *TextAreaView, logical: LogicalCursor) ?ResolvedCursor {
        const lines = self.layout.lines();
        if (lines.len == 0) {
            return .{
                .absolute_row = 0,
    ...
    fn verticalMarginRows(self: *const TextAreaView) u32 {
        const height = self.viewport.height_rows;
    ...
    fn clampViewport(self: *TextAreaView) void {
...
test "TextAreaView wrapped visual up down preserves desired visual column" {
    var buffer = EditBuffer.init(testing.allocator, .wcwidth);
    defer buffer.deinit();
    buffer.setText("abcd efgh ijkl");

    var view = TextAreaView.init(testing.allocator, &buffer);
    defer view.deinit();
...
test "TextAreaView ensureCursorVisible keeps cursor inside scroll margins" {
    var buffer = EditBuffer.init(testing.allocator, .wcwidth);
    defer buffer.deinit();
...
test "TextAreaView resize reflows once and keeps cursor visible" {
    var buffer = EditBuffer.init(testing.allocator, .wcwidth);
    defer buffer.deinit();
