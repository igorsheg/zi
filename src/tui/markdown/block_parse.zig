// Ported from vercel-labs/fx src/core/agent/presentation/block_parse.zig at commit 5ed3be1.
...
pub const ParsedBlockquote = struct {
    indent: []const u8,
    depth: usize,
    content: []const u8,
};
...
pub const ParsedUnorderedList = struct {
    indent: []const u8,
    content: []const u8,
};
...
pub const ParsedOrderedList = struct {
    indent: []const u8,
    marker: []const u8,
    content: []const u8,
};
...
test "code fence markers" {
    try std.testing.expectEqual(@as(?u8, '`'), codeFenceMarker("```zig"));
    try std.testing.expectEqual(@as(?u8, '~'), codeFenceMarker("~~~"));
    try std.testing.expectEqual(@as(?u8, null), codeFenceMarker("``"));
    try std.testing.expectEqualStrings("zig", codeFenceLanguage("```  zig extra"));
    try std.testing.expect(hasIndentedCodePrefix("    x"));
    try std.testing.expectEqualStrings("x", deindentCodeLine("\tx"));
}

test "header and setext parsing" {
    const header = parseHeader("### Title").?;
    try std.testing.expectEqual(@as(usize, 3), header.level);
    try std.testing.expectEqualStrings("Title", header.content);
    try std.testing.expectEqual(@as(?usize, 2), parseSetextUnderline("---"));
    try std.testing.expect(isSetextCandidate("Title"));
    try std.testing.expect(!isSetextCandidate("# Not"));
}

test "blockquote parsing" {
    const quote = parseBlockquote("  > deep >> x") orelse return error.TestUnexpectedResult;
    try std.testing.expectEqual(@as(usize, 2), quote.indent.len);
    try std.testing.expectEqual(@as(usize, 1), quote.depth);
    try std.testing.expect(isLazyBlockquoteContinuation("plain text"));
    try std.testing.expect(!isLazyBlockquoteContinuation("> quoted"));
}

test "list parsing" {
    const unordered = parseUnorderedList("- item") orelse return error.TestUnexpectedResult;
    try std.testing.expectEqualStrings("item", unordered.content);
    const ordered = parseOrderedList("  12. item") orelse return error.TestUnexpectedResult;
    try std.testing.expectEqualStrings("12.", ordered.marker);
    try std.testing.expectEqualStrings("item", ordered.content);
}

test "task list item parsing" {
    const done = parseTaskListItem("[x] ship it") orelse return error.TestUnexpectedResult;
    try std.testing.expect(done.completed);
    try std.testing.expect(done.has_separator);
    try std.testing.expectEqualStrings("ship it", done.content);
    try std.testing.expectEqual(@as(?ParsedTaskListItem, null), parseTaskListItem("[z] no"));
}

test "horizontal rule and pipes" {
    try std.testing.expect(isHorizontalRule("--- -"));
    try std.testing.expect(!isHorizontalRule("--"));
    try std.testing.expect(isPipeLine("a | b"));
    try std.testing.expect(!isPipeLine("a \\| b"));
}

test "valid table requires separator on line two" {
    try std.testing.expect(isValidTable("a | b\n--- | ---\n1 | 2\n"));
    try std.testing.expect(!isValidTable("a | b\n1 | 2\n"));
}

test "footnote definition" {
    const def = parseFootnoteDefinition("[^1]: body") orelse return error.TestUnexpectedResult;
    try std.testing.expectEqualStrings("1", def.label);
    try std.testing.expectEqualStrings("body", def.body);
    try std.testing.expectEqualStrings("cont", footnoteContinuationBody("  cont").?);
    try std.testing.expectEqualStrings("body", definitionMarkerBody(": body").?);
}
