// Ported from vercel-labs/fx src/core/agent/presentation/block_render.zig at commit 5ed3be1.
...
pub fn writeBlockquoteLine(
    ...
    try inline_render.writeInline(
        alloc,
        tu.withoutTerminalHardBreakMarker(content, line_has_lf),
        out,
        false,
        footnotes,
        link_id,
...
pub fn writeDefinitionLine(
    ...
    try inline_render.writeInline(
        alloc,
        tu.withoutTerminalHardBreakMarker(body, line_has_lf),
        out,
        false,
        footnotes,
        link_id,
...
fn parseAndRenderRows(
    ...
    while (start < buf.len) {
        ...
        try splitCells(alloc, buf[start..end], &raw);
...
fn splitCells(alloc: Allocator, line: []const u8, cells: *std.ArrayList([]const u8)) !void {
...
fn parseColumnAlignments(alloc: Allocator, buf: []const u8, alignments: []payload.TableColumnAlign) void {
    ...
    splitCells(alloc, sep_line, &cells) catch return;
...
test "renderTable aligns columns by separator row" {
    var out: std.ArrayList(u8) = .empty;
    defer out.deinit(std.testing.allocator);
    var link_id: u32 = 0;
    try renderTable(std.testing.allocator, "h | hh\n--- | --:\n1 | 2\n", &out, null, &link_id);
    var lines = std.mem.splitScalar(u8, out.items, '\n');
    const header = lines.next().?;
    const sep = lines.next().?;
    const body = lines.next().?;
    try std.testing.expect(std.mem.indexOf(u8, header, "hh") != null);
    // Right-aligned body cell in column two.
    try std.testing.expect(std.mem.endsWith(u8, body, "2"));
    try std.testing.expect(std.mem.indexOf(u8, body, " \xe2\x94\x82  2") != null);
    for (sep) |c| {
        try std.testing.expect(c == ' ' or c == '\xe2' or c == '\x94' or c == '\xbc' or c == '\x80');
    }
}

test "parseTablePayload round-trips cells and alignments" {
    var link_id: u32 = 0;
    var table = try parseTablePayload(std.testing.allocator, "l | r\n--- | --:\na | b\n", &link_id);
    defer table.deinit(std.testing.allocator);
    try std.testing.expectEqual(@as(usize, 2), table.rows.len);
    try std.testing.expectEqual(@as(usize, 2), table.column_count);
    try std.testing.expectEqual(payload.TableColumnAlign.right, table.alignments[1]);
    try std.testing.expectEqualStrings("b", table.rows[1].cells[1]);
}

test "renderTablePayload pads right-aligned cells" {
    var out: std.ArrayList(u8) = .empty;
    defer out.deinit(std.testing.allocator);
    const rows = [_]payload.TableRow{
        .{ .cells = @constCast(&[_][]u8{ @constCast("ab"), @constCast("c") }) },
    };
    var alignments = [_]payload.TableColumnAlign{ .left, .right };
    try renderTablePayload(std.testing.allocator, .{
        .rows = @constCast(&rows),
    ...
    // Header cells are bold wrapped; column two stays padded to width one.
    try std.testing.expect(std.mem.indexOf(u8, line, " \x1b[1mc\x1b[22m") != null);
}

test "heading styles close their runs" {
    var out: std.ArrayList(u8) = .empty;
    defer out.deinit(std.testing.allocator);
    var link_id: u32 = 0;
    try writeHeading(std.testing.allocator, 1, "title", &out, null, &link_id);
    try std.testing.expect(std.mem.startsWith(u8, out.items, ansi.bold_open ++ ansi.underline_open));
    try std.testing.expect(std.mem.endsWith(u8, out.items, ansi.underline_close ++ ansi.bold_close));
}

test "task list marker reflects completion state" {
    var out: std.ArrayList(u8) = .empty;
    defer out.deinit(std.testing.allocator);
    try writeTaskListMarker(std.testing.allocator, &out, .{ .completed = true, .has_separator = true, .content = "" });
    try std.testing.expect(std.mem.indexOf(u8, out.items, ansi.task_completed_marker) != null);

    var pending: std.ArrayList(u8) = .empty;
    defer pending.deinit(std.testing.allocator);
    try writeTaskListMarker(std.testing.allocator, &pending, .{
        .completed = false,
        .has_separator = true,
        .content = "",
    });
    try std.testing.expect(std.mem.indexOf(u8, pending.items, ansi.task_pending_marker) != null);
}
