// Ported from vercel-labs/fx src/core/agent/presentation/text_util.zig at commit 5ed3be1.
...
test "leftTrim and blank detection" {
    try std.testing.expectEqualStrings("a ", leftTrim(" \ta "));
    try std.testing.expect(isBlankMarkdownLine("  \t "));
    try std.testing.expect(!isBlankMarkdownLine(" x"));
}

test "escaped punctuation" {
    try std.testing.expect(isEscapedPunctuationAt("a\\*", 2));
    try std.testing.expect(!isEscapedPunctuationAt("a\\\\*", 3));
    var out: std.ArrayList(u8) = .empty;
    defer out.deinit(std.testing.allocator);
    try appendEscapedPunctuation(std.testing.allocator, &out, "a\\*b");
    try std.testing.expectEqualStrings("a*b", out.items);
}

test "underscore validity" {
    try std.testing.expect(isValidUnderscoreOpen("_a", 0, 1));
    try std.testing.expect(!isValidUnderscoreOpen("a _ a", 2, 1));
    try std.testing.expect(isValidUnderscoreClose("a_", 1, 1));
    try std.testing.expect(!isValidUnderscoreClose("a _ ", 2, 1));
    try std.testing.expectEqual(@as(?usize, 5), findUnderscoreCloser("_ab c_", 1, 1));
}

test "nthLine" {
    const buf = "one\ntwo\nthree";
    try std.testing.expectEqualStrings("one", nthLine(buf, 0).?);
    try std.testing.expectEqualStrings("three", nthLine(buf, 2).?);
    try std.testing.expectEqual(@as(?[]const u8, null), nthLine(buf, 3));
}

test "withoutTerminalHardBreakMarker" {
    try std.testing.expectEqualStrings("a\\", withoutTerminalHardBreakMarker("a\\", false));
    try std.testing.expectEqualStrings("a", withoutTerminalHardBreakMarker("a\\", true));
    try std.testing.expectEqualStrings("a\\\\", withoutTerminalHardBreakMarker("a\\\\", true));
}
